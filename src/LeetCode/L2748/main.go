...
*/
func main() {
	fmt.Println("countBeautifulPairs([]int{756,1324,2419,495,106,111,1649,1474,2001,1633,273,1804,2102,1782,705,1529,1761,1613,111,186,412}) = ", countBeautifulPairs([]int{756, 1324, 2419, 495, 106, 111, 1649, 1474, 2001, 1633, 273, 1804, 2102, 1782, 705, 1529, 1761, 1613, 111, 186, 412}))
}

func countBeautifulPairs(nums []int) int {
	var fl [][]int
	for _, num := range nums {
		fl = append(fl, getFirstAndLast(num))
	}
	le := len(nums)
	var result int
	for i := 0; i < le; i++ {
		for j := i + 1; j < le; j++ {
			if gcd(fl[i][0], fl[j][1]) == 1 {
				result++
			}
		}
	}
...
func getFirstAndLast(a int) []int {
	result := []int{0, a % 10}
	for a >= 10 {
		a /= 10
	}
