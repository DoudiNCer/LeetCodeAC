...
*/
func main() {
	fmt.Println("minimumPushes(\"xyzxyzxyzxyz\") = ", minimumPushes("xyzxyzxyzxyz"))
}
...
func minimumPushes(word string) int {
	...
	result := 0
	for i := 0; i < 8 && cnts[i] > 0; i++ {
		result += cnts[i]
	}
	for i := 8; i < 16 && cnts[i] > 0; i++ {
		result += cnts[i] * 2
	}
	for i := 16; i < 24 && cnts[i] > 0; i++ {
		result += cnts[i] * 3
	}
	for i := 24; i < 26 && cnts[i] > 0; i++ {
		result += cnts[i] * 4
	}
	return result
}
